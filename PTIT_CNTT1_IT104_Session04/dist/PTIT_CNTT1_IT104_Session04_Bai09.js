...
const calculateInvoiceTotal = (invoice) => {
    ...
    orders.forEach((order) => {
        let totalPrice = 0;
        totalPrice = order.items.reduce((sum, item) => sum + item.quantity * item.product.price, 0);
        total += totalPrice;
    });
    return total;
};
...
const printInvoice = (invoice) => {
    ...
    for (let i = 0; i < orders.length; i++) {
        ...
        for (let j = 0; j < orders[i].items.length; j++) {
            console.log(`
        - ${orders[i].items[j].product.name} * ${orders[i].items[j].quantity} -> ${orders[i].items[j].product.price.toLocaleString("vi-VN")}
        `);
        }
    ...
    console.log(`        Tong don hang la: ${calculateInvoiceTotal(invoice).toLocaleString("vi-VN")}`);
};
